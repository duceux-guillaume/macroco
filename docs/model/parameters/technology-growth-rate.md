...
The original World3 standard run (BAU) assumes no improvement in industrial productivity. This was a deliberate modeling choice by Meadows et al. — the BAU scenario asks "what happens with no policy intervention and no lucky technological breakthroughs?"
...
The BAU rate of 1.4%/yr was calibrated against World Bank IOPC data (1960-2023) to achieve RMSE below 19% (REQ-026). The ISOPC dynamic lookup (March 2026) captures service-capital feedback that was previously absorbed by the technology rate, allowing it to be reduced from earlier calibration values. This parameter is highly sensitive: values above 2% cause a qualitative bifurcation (population peak shifts from ~2030 to ~2073).
...
- At $$g = 0$$, the model reproduces the original World3 BAU trajectory with an earlier and sharper collapse.
...
Always run `cargo test -p world3-cli --test qualitative_dynamics` after changing this parameter to verify that the BAU overshoot-and-collapse shape is preserved.
