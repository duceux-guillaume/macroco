 ...
 *
 * Source: docs/model/ (sectors, parameters, feedback-loops)
 * Generator: scripts/generate-variable-descriptions.py
 */
...
export const variableDescriptions: Record<string, VariableInfo> = {
	'agriculture.arable_land': {
		...
		expert:
			'd(AL)/dt = development_rate - erosion_rate. Development limited by potentially_arable_land and investment. Erosion = AL x 0.002 x erosion_mult(yield_ratio) x (1 - land_protection).',
		feedbackLoops: ['pollution-food'],
		relatedVariables: ['agriculture.food', 'agriculture.food_per_capita', 'agriculture.land_yield'],
		docPath: 'docs/model/sectors/agriculture.md'
	},
	'agriculture.urban_industrial_land': {
		...
		expert:
			'First-order delay converging to UILPC(IOPC) x POP with time constant UILD=10yr. Growth subtracts from arable land stock, constrained by available arable land.',
		feedbackLoops: ['pollution-food'],
	...
	'agriculture.land_fertility': {
		...
		expert:
			'd(LFERT)/dt = LFR - LFD. LFD = LFERT x LFDR(pollution_index). LFR = (ILF - LFERT) / LFRT(FALM(food_ratio)). ILF = 600 kg/ha/yr. World3-03 ODE stock.',
		feedbackLoops: ['pollution-food'],
	...
	'agriculture.food': {
		...
		expert:
			'food = arable_land x land_yield. land_yield = 600 x yield_mult_capital(inputs/ha) x yield_mult_pollution(poll_index) x agri_technology.',
	...
	'agriculture.food_per_capita': {
		...
		feedbackLoops: ['pollution-food', 'food-population'],
		relatedVariables: ['agriculture.food', 'population.population', 'population.life_expectancy'],
		docPath: 'docs/model/sectors/agriculture.md'
	},
	'agriculture.land_yield': {
		...
		expert:
			'LY = land_fertility x LYMC(agri_inputs/ha) x LYMAP(pollution_index) x ag_tech, where ag_tech = agricultural_technology x (1 + ag_tech_growth_rate)^max(year-1960, 0). Base fertility 600 kg/ha/yr (1900). LYMC ranges 1.0->10.0. LYMAP ranges 1.0->0.40.',
		feedbackLoops: ['pollution-food'],
	...
	'capital.perceived_iopc': {
		...
		docPath: 'docs/model/sectors/capital.md'
	},
	'capital.industrial_capital': {
		...
		expert:
			'd(IC)/dt = industrial_output x frac_to_investment - IC x depreciation_rate. Investment is the residual after consumption, services, and agriculture allocation.',
	...
	'capital.industrial_output': {
		...
		expert:
			'IO = productive_capital / ICOR. productive_capital = IC x (1 - capital_for_resources) x tech_multiplier. ICOR = 3.0 x COR_resources(fraction_remaining).',
	...
	'capital.industrial_output_per_capita': {
		...
		feedbackLoops: ['resource-collapse', 'demographic-transition'],
		relatedVariables: ['capital.industrial_output', 'population.population', 'population.fertility_rate'],
		docPath: 'docs/model/sectors/capital.md'
	},
	...
	'pollution.persistent_pollution': {
		...
		expert:
			'd(PP)/dt = generation - assimilation. generation = (gen_industry + gen_agriculture) x (1 - pollution_control). assimilation = PP / assimilation_time(PP).',
		feedbackLoops: ['pollution-tipping', 'pollution-food'],
		relatedVariables: ['pollution.pollution_index', 'capital.industrial_output'],
		docPath: 'docs/model/sectors/pollution.md'
	},
	...
	'pollution.pollution_index': {
		...
		feedbackLoops: ['pollution-tipping', 'pollution-food'],
		relatedVariables: ['pollution.persistent_pollution', 'population.life_expectancy', 'agriculture.land_yield'],
		docPath: 'docs/model/sectors/pollution.md'
	},
	'population.population': {
		...
		docPath: 'docs/model/sectors/population.md'
	},
	'population.life_expectancy': {
		...
		expert:
			'life_expectancy = 28.0 x LEM_food(food_ratio) x LEM_health(HSAPC) x LMC(CMI,FPU) x LEM_pollution(pollution_index). Product of four lookup-table multipliers on a 28-year base. Health services per capita from World3-03 HSAPC table (maps SOPC to health spending directly).',
		feedbackLoops: ['food-population', 'pollution-food'],
		relatedVariables: ['population.population', 'agriculture.food_per_capita', 'pollution.pollution_index'],
		docPath: 'docs/model/sectors/population.md'
	},
	'population.birth_rate': {
		name: 'Birth Rate',
		...
		expert:
			'birth_rate = births_per_year / population. births_per_year = (cohort_15_44 x 0.5) x total_fertility_rate / 30.0. TFR = desired_family_size(iopc) x fp_multiplier(efficacy x ramp) x food_fertility(food_ratio).',
	...
	'population.death_rate': {
		...
		expert:
			'death_rate = total_deaths / population. Deaths per cohort use age-weighted base mortality (1/life_expectancy x age_factor).',
		feedbackLoops: ['food-population', 'pollution-food'],
	...
	'population.fertility_rate': {
		...
		expert:
			'TFR = desired_family_size(iopc) x family_planning_multiplier(efficacy x ramp) x food_fertility_multiplier(food_ratio). Desired family size ranges from 5.0 at iopc=0 to 1.9 at iopc=1600.',
		feedbackLoops: ['demographic-transition'],
	...
	'resources.nonrenewable_resources': {
		...
		expert:
			'd(NNR)/dt = -population x IOPC x 3.0e-15 / resource_efficiency. Calibrated so 1970 extraction ~ 0.54%/yr of initial stock.',
		...
		docPath: 'docs/model/sectors/resources.md'
	},
	'resources.fraction_remaining': {
		...
		relatedVariables: ['resources.nonrenewable_resources', 'capital.industrial_output', 'capital.industrial_capital'],
		docPath: 'docs/model/sectors/resources.md'
	}
};
...
export const parameterDescriptions: Record<string, ParameterInfo> = {
	agricultural_technology_growth_rate: {
		name: 'Agricultural Technology Growth Rate',
		unit: 'yr⁻¹',
		sector: 'Agriculture',
		beginner:
		...
		docPath: 'docs/model/parameters/agricultural-technology-growth-rate.md'
	},
	agricultural_technology: {
		...
		expert:
			'Direct multiplier on land_yield: LY = 600 × LYMC × LYMAP × agri_tech.',
		feedbackLoops: ['food-population', 'pollution-food'],
		...
		docPath: 'docs/model/parameters/agricultural-technology.md'
	},
	family_planning_efficacy: {
		...
		docPath: 'docs/model/parameters/family-planning-efficacy.md'
	},
	family_planning_year: {
		name: 'Family Planning Year',
		unit: 'year',
		sector: 'Population',
		beginner:
		...
		docPath: 'docs/model/parameters/family-planning-year.md'
	},
	health_investment_multiplier: {
		...
		expert:
			'Scales service_output_per_capita input to life_exp_multiplier_health lookup.',
		feedbackLoops: ['demographic-transition', 'food-population'],
		...
		docPath: 'docs/model/parameters/health-investment-multiplier.md'
	},
	industrial_depreciation_rate: {
		name: 'Industrial Capital Depreciation Rate',
		unit: 'fraction/yr',
		sector: 'Capital',
		beginner:
		...
		docPath: 'docs/model/parameters/industrial-depreciation-rate.md'
	},
	initial_nnr_fraction: {
		name: 'Initial NNR Fraction',
		unit: 'fraction (0--1)',
		sector: 'Resources',
		beginner:
		...
		expert:
			'Initial condition for nonrenewable_resources ODE stock.',
		feedbackLoops: ['resource-collapse'],
		relatedVariables: [
			'resources.nonrenewable_resources',
			'resources.fraction_remaining',
		...
		docPath: 'docs/model/parameters/initial-nnr-fraction.md'
	},
	investment_rate: {
		...
		expert:
			'Fraction of industrial_output allocated to gross investment. d(IC)/dt investment term = IO x investment_rate.',
		feedbackLoops: ['resource-collapse'],
		relatedVariables: [
			'capital.industrial_capital',
			'capital.industrial_output',
		...
		docPath: 'docs/model/parameters/investment-rate.md'
	},
	land_protection_fraction: {
		name: 'Land Protection Fraction',
		unit: 'fraction (0--0.5)',
		sector: 'Agriculture',
		beginner:
		...
		docPath: 'docs/model/parameters/land-protection-fraction.md'
	},
	pollution_control: {
	...
	resource_efficiency: {
		...
		docPath: 'docs/model/parameters/resource-efficiency.md'
	},
	service_depreciation_rate: {
		name: 'Service Capital Depreciation Rate',
		unit: 'fraction/yr',
		sector: 'Capital',
		beginner:
		...
		docPath: 'docs/model/parameters/service-depreciation-rate.md'
	},
	subsistence_food_per_capita: {
		...
		docPath: 'docs/model/parameters/subsistence-food-per-capita.md'
	},
	technology_growth_rate: {
		...
		docPath: 'docs/model/parameters/technology-growth-rate.md'
	}
};
...
export const feedbackLoops: Record<string, FeedbackLoopInfo> = {
	'resource-collapse': {
		...
		docPath: 'docs/model/feedback-loops.md'
	},
	'pollution-food': {
		id: 'pollution-food',
		name: 'Pollution → Agricultural Decline → Food Crisis',
		type: 'reinforcing',
		description:
